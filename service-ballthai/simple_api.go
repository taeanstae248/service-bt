...
func startAPIServer() {
	...
	http.HandleFunc("/api/leagues", corsHandler(getLeagues))
	http.HandleFunc("/api/teams", corsHandler(getTeams))
	http.HandleFunc("/api/stadiums", corsHandler(getStadiums))
	http.HandleFunc("/api/matches", corsHandler(getMatches))

	// Handle static files
	http.Handle("/images/", http.StripPrefix("/images/", http.FileServer(http.Dir("./img/"))))
...
func getTeams(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		SELECT t.id, t.name_th as name, t.team_post_ballthai, t.stadium_id, s.name as stadium_name, 
		       t.logo_url as logo, NULL as established_year
		FROM teams t 
...
func getMatches(w http.ResponseWriter, r *http.Request) {
	...
	query := `
		SELECT m.id, ht.name_th as home_team, at.name_th as away_team,
		       ...
		       m.league_id, l.name as league_name, hs.name as stadium_name
		FROM matches m
		JOIN teams ht ON m.home_team_id = ht.id
		JOIN teams at ON m.away_team_id = at.id
		JOIN leagues l ON m.league_id = l.id
		LEFT JOIN stadiums hs ON ht.stadium_id = hs.id
		WHERE 1=1
	`
